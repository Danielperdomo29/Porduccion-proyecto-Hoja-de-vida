...
// ===== DETECCIÓN AVANZADA DE CONTEXTO =====
const patronesOfensivos = require("../config/patronesOfensivos.json");

function detectarMetaforasSexuales(texto) {
  const textoNorm = normalizeForMatch(texto).toLowerCase();
  const textoOriginal = texto.toLowerCase();

  for (const [palabra, config] of Object.entries(patronesOfensivos.metaforasSexuales)) {
    const palabraNorm = normalizeForMatch(palabra);

    if (textoNorm.includes(palabraNorm) || textoOriginal.includes(palabra)) {
      for (const contexto of config.contexto) {
        const contextoNorm = normalizeForMatch(contexto);
        if (textoNorm.includes(contextoNorm) || textoOriginal.includes(contexto)) {
          return {
            match: palabra,
            contexto: contexto,
            tipo: "metafora_sexual",
            severidad: config.severidad,
            detected: `${palabra} + ${contexto}`
          };
        }
      }
    }
...
function detectarFrasesOfensivas(texto) {
  const textoNorm = normalizeForMatch(texto).toLowerCase();

  for (const patron of patronesOfensivos.patronesSospechosos) {
    try {
      const regex = new RegExp(patron, 'i');
      if (regex.test(texto) || regex.test(textoNorm)) {
        return {
          match: patron,
          tipo: "frase_ofensiva_regex",
          severidad: "alta",
          detected: texto.substring(0, 50)
        };
      }
    } catch (e) {
      console.error("Error en regex:", patron, e);
    }
  }

  for (const frase of patronesOfensivos.frasesOfensivas) {
    try {
      ...
      console.error("Error en frase:", frase, e);
    }
  }
...
function detectarContenidoInapropiado(texto) {
  const palabrasDetectadas = detectarOfensaAvanzada(texto, palabrasProhibidas);
  if (palabrasDetectadas) return palabrasDetectadas;

  const metaforasDetectadas = detectarMetaforasSexuales(texto);
  if (metaforasDetectadas) return metaforasDetectadas;

  const frasesDetectadas = detectarFrasesOfensivas(texto);
  if (frasesDetectadas) return frasesDetectadas;
...
exports.crearComentario = async (req, res) => {
  try {
    ...
    if (!captchaValido) return res.status(400).json({ error: " Verificación reCAPTCHA fallida." });
    ...
    const deteccion = detectarContenidoInapropiado(contenido);

    if (deteccion) {
      // Metáforas sexuales y frases ofensivas => rechazo automático
      if (deteccion.tipo === "metafora_sexual" || deteccion.tipo === "frase_ofensiva" || deteccion.tipo === "frase_ofensiva_regex") {
        console.warn("Comentario rechazado (contexto ofensivo):", {
          user: req.user.id,
          tipo: deteccion.tipo,
          match: deteccion.match,
          detected: deteccion.detected
        });
        return res.status(400).json({ error: "🚫 Tu comentario contiene contenido inapropiado." });
      }
